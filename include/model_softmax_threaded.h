...
double *forward_pass(Model *model, double input[], double activation(double), int num_threads)
{
...
double backward_pass(
    ...
    double dactivation(double), int num_threads)
{
    ...
    for (int i = model->num_layers - 1; i >= 0; i--)
    {
        ...
        error[i] = mat_mul(weights_t, error[i + 1], num_threads);
