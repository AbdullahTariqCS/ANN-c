...
double mean_squared_error(Matrix *actual, Matrix *predicted)
{
    double mse = 0;
    for (int i = 0; i < actual->rows; i++)
    {
        ...
        {
            double diff = actual->matrix[i][j] - predicted->matrix[i][j];
            mse += diff * diff;
        }
    }

    // Calculate mean by dividing by the total number of elements
    mse /= (actual->rows * actual->columns);
    return mse;
}
