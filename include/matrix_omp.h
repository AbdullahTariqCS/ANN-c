...
void mat_write(Matrix *A, FILE *file)
{
    ...
    char **row_buffers = (char **)malloc(A->rows * sizeof(char *));
    int row_size = A->columns * 32; // Allocate enough space per row

#pragma omp parallel for
    for (int i = 0; i < A->rows; i++)
    {
        row_buffers[i] = (char *)malloc(row_size);
        row_buffers[i][0] = '\0';
        for (int j = 0; j < A->columns; j++)
        {
            char temp[32];
            snprintf(temp, sizeof(temp), "%f;", A->matrix[i][j]);
            strcat(row_buffers[i], temp);
        }
    }
...
// Parallelize the outer loops (i and j)
#pragma omp parallel for collapse(2)
    for (int i = 0; i < A->rows; i++)
    {
...
Matrix *arr_to_mat(int len, double *arr, int column)
{
...
{
    double *arr = (double *)malloc(A->rows * A->columns * sizeof(double));
    int k = 0;
    for (int i = 0; i < A->rows; i++)
        for (int j = 0; j < A->columns; j++)
            arr[k++] = A->matrix[i][j];
...
{
    double *normal_data = (double *)malloc(A->rows * A->columns * sizeof(double));
    for (int i = 0; i < A->rows; i++)
        for (int j = 0; j < A->columns; j++)
            A->matrix[i][j] = 0;
}
